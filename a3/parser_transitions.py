...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        self.stack = ['ROOT']
        # if we use self.buffer = self.sentence here, it will be a shallow copy
...
def minibatch_parse(sentences, model, batch_size):
    ...
    ###                 unfinished_parses = partial_parses[:].x`
    ...
    while unfinished_parses:
        ...
        options = model.predict(unfinished_parses)
        tmp = unfinished_parses.copy()
        for parse, opt in zip(tmp, options) :
            parse.parse_step(opt)
