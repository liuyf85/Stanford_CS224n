...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        tmp = [[self.embeddings[idx] for idx in sen] for sen in w]
        x = torch.stack([torch.cat(sen) for sen in tmp])        ###################### attention
    ...
    def forward(self, w):
        ...
        # attetion for multiple w !!!!!
        w = self.embedding_lookup(w)
        
        ################# 一定要用 F.relu 而不是 nn.ReLU, 要不然可能会造成不可导的情况 || 是否有丢失维度的风险！！！

        w = F.relu(torch.matmul(w, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        w = self.dropout(w)
        logits = F.relu(torch.matmul(w, self.hidden_to_logits_weight) + self.hidden_to_logits_bias)
